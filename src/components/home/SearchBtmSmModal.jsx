...
const SearchBtmSmModal = () => {
  ...
  const beds = ["All", "Bunker", "1 BR", "2 BR", "3 BR"];
  ...
  const handleBedSelection = (index) => {
    if (categoryValue === 1) {
      ...
      if (index === 2) {
        ...
        setBedrooms(["Single"]); // Automatically select "1 BR" bed for Private Room
      }
      setBedValue(index); // Update the selected bed value
    } else if (categoryValue === 2) {
      // Shared Room category
      if (beds[index] === "Bunker") {
        setBedrooms(["Bunker"]); // Set bedrooms to ["Bunker"] for Shared Room
      } else if (index === 2) {
        setBedrooms(["1"]); // Automatically select "1 BR" bed for Private Room
      } else {
          ...
          <div className="flex items-center">
            ...
            <div style={{ marginTop: "19px" }}>
              ...
              >
                {beds.map((bed, index) => {
                  if (
                    (categoryValue === 1 && bed !== "1 BR" && bed !== "2 BR") ||
                    (categoryValue === 2 &&
                      bed !== "1 BR" &&
                      bed !== "Bunker") ||
                    (categoryValue === 3 &&
